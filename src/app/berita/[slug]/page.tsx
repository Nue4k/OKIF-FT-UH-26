...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const data: Berita | null = await beritaService.getBySlug(slug);
...
export default async function BeritaSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
      ...
      <section className="w-full bg-white py-10 md:py-14 px-6 md:px-8">
        <div className="w-full max-w-[880px] mx-auto flex flex-col">
          ...
          <div className="flex items-center gap-2 mb-8 text-[#767676] text-sm font-medium">
            Oleh: <span className="text-[#0051FF] font-bold">{data.author}</span>
          </div>
